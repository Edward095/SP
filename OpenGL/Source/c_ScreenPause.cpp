...
void c_ScreenPause::Init()
{
	...
	meshList[TEXT]->textureID = LoadTGA("Image//PauseScreen.tga");
	meshList[ARROW] = MeshBuilder::GenerateQuad("Arrow", Color(1, 0, 0), 0.7f);
	meshList[ARROW]->textureID = LoadTGA("Image//Arrow.tga");

}
void c_ScreenPause::Update(double dt)
{
	elapsedTime += dt;

	switch (e_GameState)
	{
	case c_ScreenPause::MENU:
		updateSelection(dt);
		break;
	case c_ScreenPause::CONTINUE:
		updateContinue(dt);
		break;
	case c_ScreenPause::EXIT:
		break;
	default:
		break;
	}
...
void c_ScreenPause::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void c_ScreenPause::renderLights()
{
	//Lights 1
...
void c_ScreenPause::updateLights(int num)
{
	if (num == 0)
	{
...
void c_ScreenPause::renderSelection()
{
...
void c_ScreenPause::updateSelection(double dt)
{
	if (Application::IsKeyPressed(VK_UP) && bounceTime < elapsedTime) // up to continue
	{
		if (ArrowY < -1.7f)
		{
			ArrowY += 1.8f;
	...
	if (Application::IsKeyPressed(VK_DOWN) && bounceTime < elapsedTime) // down to exit
	{
		if (ArrowY > -1.7f)
		{
			ArrowY -= 1.8f;
	...
	if (Application::IsKeyPressed(VK_SPACE)) // selection
	{
		if (ArrowY == -1.6f)
...
void c_ScreenPause::renderContinue()
{
	// Link to scene for continue
}
void c_ScreenPause::updateContinue(double dt)
{
